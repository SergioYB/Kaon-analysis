...
void k0_ksl_decay(){
    ...
    for(int i_e = 0; i_e < n_events; i_e++) {
        ...
        //Loop over g4 particles to finds primary ksl produced by resonance decay on a primary k0
        for(int i_p = 0; i_p < size_g4; i_p++){
            //Find Ks and Kl
            if((g4_part_PDGcode->at(i_p) == 310) & (nu_interaction_mode == 1)){
                if (g4_part_process->at(findPos(g4_part_trackID, g4_part_mother->at(i_p))) == "primary"){
                    //Filter only if ks end in decay
                    if (g4_part_end_process->at(i_p) == "Decay"){

                        //Print ks g4 info
                        cout << endl << "========== KS IN EVENT " << i_e << " ================" << endl;
                        cout << " ID: " << g4_part_trackID->at(i_p) << endl;
                        cout << "Initial energy: " << g4_part_E0->at(i_p) << endl;
                        ...
                        cout << "End process: " << g4_part_end_process->at(i_p) << endl;


                        //Loop over g4 particles to find k0 children (start pos of children = end pos of kaon)
                        for (int j_p = 0; j_p < size_g4; j_p++){
                            if ((g4_part_mother->at(j_p) == g4_part_trackID->at(i_p)) & (g4_part_start_pos_X->at(j_p) == g4_part_end_pos_X->at(i_p))){
                                //Print children info
                                cout << endl;
                                cout << "\t Chldrn PDG: " << g4_part_PDGcode->at(j_p) << endl;
                                cout << "\t Chldrn end process: " << g4_part_end_process->at(j_p) << endl;
            ...
            } else if ((g4_part_PDGcode->at(i_p) == 130) & (nu_interaction_mode == 1)){
                if (g4_part_process->at(findPos(g4_part_trackID, g4_part_mother->at(i_p))) == "primary"){

                    //Filter only if kl end in decay
                    if (g4_part_end_process->at(i_p) == "Decay"){

                        //Print ks g4 info
                        cout << endl << "========== KL IN EVENT " << i_e << " ================" << endl;
                        cout << "Start Pos (x,y,z) : (" << g4_part_start_pos_X->at(i_p) << ", " << g4_part_start_pos_Y->at(i_p) << ", " << g4_part_start_pos_Z->at(i_p)  << ")" << endl;
                        cout << "End Pos (x,y,z) : (" << g4_part_end_pos_X->at(i_p) << ", " << g4_part_end_pos_Y->at(i_p) << ", " << g4_part_end_pos_Z->at(i_p)  << ")" << endl;
                        cout << "End process: " << g4_part_end_process->at(i_p) << endl;
