...
void traces_pipm_p(){
    ...
    TH2F *h_k0 = new TH2F("h_k0", "", 4, 0.5 , 4.5, 4, 0.5, 4.5); //x-> 0,1,2+ traces ; y-> 0,1,2+ pi0
    TH2F *h_k0ak0 = new TH2F("h_k0ak0", "", 4, 0.5 , 4.5, 4, 0.5, 4.5); 


    //Create partial histograms to be added as a single one
    TH2F *h_B0 = new TH2F("h_B0", "", 4, 0.5 , 4.5, 4, 0.5, 4.5); 
    TH2F *h_Bpm = new TH2F("h_Bpm", "", 4, 0.5 , 4.5, 4, 0.5, 4.5);  
    TH2F *h_noB = new TH2F("h_noB0", "", 4, 0.5 , 4.5, 4, 0.5, 4.5);  
    ...
    for (size_t i_v = 0; i_v < events_k0.size(); ++i_v){
        ...
        for (int j_p = 0; j_p < size_g4; j_p++){
            ...
            if (g4_part_process->at(j_p) == "primary"){ //Both conditions equivalent?
                ...
                if (g4_part_PDGcode->at(j_p) == 2212){
                    n_p++;
                    par_found = true;
                }  else if ((std::abs(g4_part_PDGcode->at(j_p)) == 211)){
                    n_pic++;
                    par_found = true;
                }
        ...
        //Fill histograms
        h_k0->Fill(fmin(n_pic + 1, 4),fmin(n_p + 1, 4));

        if (n_B0){
            h_B0->Fill(fmin(n_pic + 1, 4),fmin(n_p + 1, 4));
        } else if (n_Bpm){
            h_Bpm->Fill(fmin(n_pic + 1, 4),fmin(n_p + 1, 4));
        } else {
            h_noB->Fill(fmin(n_pic + 1, 4),fmin(n_p + 1, 4));
        }
    }
    ...
    h_k0->GetXaxis()->SetTitle("#pi^{#pm}");
    h_k0->GetYaxis()->SetTitle("p");
    ...
    h_B0->GetXaxis()->SetTitle("#pi^{#pm}");
    h_B0->GetYaxis()->SetTitle("p");
    ...
    h_Bpm->GetXaxis()->SetTitle("#pi^{#pm}");
    h_Bpm->GetYaxis()->SetTitle("p");
    ...
    h_noB->GetXaxis()->SetTitle("#pi^{#pm}");
    h_noB->GetYaxis()->SetTitle("p");
    ...
    */
    ...
    for (size_t i_v = 0; i_v < events_k0ak0.size(); ++i_v){
        ...
        //Fill histograms
        h_k0ak0->Fill(fmin(n_pic + 1, 4),fmin(n_p + 1, 4));
    ...
    h_k0ak0->GetXaxis()->SetTitle("#pi^{#pm}");
    h_k0ak0->GetYaxis()->SetTitle("p");
