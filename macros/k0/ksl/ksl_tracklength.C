...
void ksl_tracklength(){
    ...
    TH1F *h_ks_in = new TH1F("h_ks_in", "", nbins_s, &bins_s[0]); 
    TH1F *h_ks_out = new TH1F("h_ks_out", "", nbins_s, &bins_s[0]);
    TH1F *h_ks_in_small = new TH1F("h_ks_in_small", "", 15, 0, 4); 
    TH1F *h_ks_out_small = new TH1F("h_ks_out_small", "", 15, 0, 4);
    TH1F *h_kl_in = new TH1F("h_kl_in", "", nbins_l, &bins_l[0]);  
    TH1F *h_kl_out = new TH1F("h_kl_out", "", nbins_l, &bins_l[0]); 
                ...
                for (int j_p = 0; j_p < size_g4; j_p++){
                    ...
                    if ((g4_part_mother->at(j_p) == g4_part_trackID->at(i_p)) & (g4_part_PDGcode->at(j_p) == 310)){
                        if (ends_inside_detector(event_tree, j_p, fv)){
                            h_ks_in->Fill(trackLength(event_tree, j_p));
                            h_ks_in_small->Fill(trackLength(event_tree, j_p));
                        } else {
                            h_ks_out->Fill(trackLength(event_tree, j_p));
                            h_ks_out_small->Fill(trackLength(event_tree, j_p));
                        }
                    ...
                    if ((g4_part_mother->at(j_p) == g4_part_trackID->at(i_p)) & (g4_part_PDGcode->at(j_p) == 130)){
                        if (ends_inside_detector(event_tree, j_p, fv)){
                            h_kl_in->Fill(trackLength(event_tree, j_p));
                        } else {
                            h_kl_out->Fill(trackLength(event_tree, j_p));
                        }
    ...
    h_ks_in_small->SetLineColor(kBlue);
    h_ks_in_small->SetLineWidth(3);
    h_ks_in_small->SetFillColor(kBlue-9);
    h_ks_out_small->SetLineColor(kRed);
    h_ks_out_small->SetLineWidth(3);
    h_ks_out_small->SetFillColor(kRed-9);
    
    h_kl_in->SetLineColor(kBlue);
    h_kl_in->SetLineWidth(3);
    h_kl_in->SetFillColor(kBlue-9);
    h_kl_out->SetLineColor(kRed);
    h_kl_out->SetLineWidth(3);
    h_kl_out->SetFillColor(kRed-9);
    ...
    hs_ks_small->Add(h_ks_in_small);
    hs_ks_small->Add(h_ks_out_small);
    ...
    legend_l->AddEntry(h_kl_in, "In", "f");
    legend_l->AddEntry(h_kl_out, "Out", "f");
    legend_l->Draw();
